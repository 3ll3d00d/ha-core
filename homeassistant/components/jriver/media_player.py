...
async def async_setup_entry(
    ...
    to_add = [
        ...
        and not entity_registry.async_get_entity_id(
            Platform.MEDIA_PLAYER, DOMAIN, e.unique_id
        )
    ]
...
class JRiverMediaPlayer(MediaServerEntity, MediaPlayerEntity):
    ...
    @cmd
    async def async_append_search_results_to_playlist(self, query: str | None):
        """Add the results of a query to playing now.

        Used by the exposed service "append_search_results_to_playlist"
        """
        if query:
            await self._media_server.play_search(
                query, zone=self._target_zone, play_next=False
            )
            return
    ...
    async def async_play_playlist(self, playlist_path: str | None):
        ...
        _LOGGER.warning("Service play_playlist requires a playlist_path to be set")
