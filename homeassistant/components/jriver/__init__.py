...
RECONFIGURE_PLATFORMS = [Platform.MEDIA_PLAYER, Platform.REMOTE, Platform.SENSOR]
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    ms_coordinator = MediaServerUpdateCoordinator(hass, entry, ms, extra_fields)
    ...
    device_zones = []
    if CONF_DEVICE_ZONES in entry.data:
        device_zones = entry.data[CONF_DEVICE_ZONES]
    if CONF_DEVICE_ZONES in entry.options:
        _LOGGER.debug("[%s] Using device zones from options", entry.entry_id)
        device_zones = entry.options[CONF_DEVICE_ZONES]
...
async def reconfigure_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    if hass.data[DOMAIN][entry.entry_id][DATA_ZONES]:
        await hass.config_entries.async_forward_entry_setups(entry, PLATFORMS)
