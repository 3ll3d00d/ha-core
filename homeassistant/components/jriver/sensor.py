    ...
    for z in zones:
        entities.extend(
            [
                ...
                JRiverAudioPathSensor(
                    data[DATA_COORDINATOR],
                    f"{uid_prefix}_{z}_audiopath",
                    f"{name} - {z} (Audio Path)",
                    z.name,
                ),
...
class JRiverAudioPathSensor(MediaServerEntity, SensorEntity):
    """Exposes detailed information about the audio path in a given zone."""
    ...
    def _handle_coordinator_update(self) -> None:
        """Handle updated data from the coordinator."""
        info = self.coordinator.data.get_audio_path(self._zone_name)
        if not info:
            return
