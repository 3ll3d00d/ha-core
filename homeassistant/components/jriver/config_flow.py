...
class JRiverConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_macs(self, user_input=None) -> ConfigFlowResult:
    ...
    async def async_step_paths(self, user_input=None) -> ConfigFlowResult:
    ...
    async def async_step_zones(self, user_input=None) -> ConfigFlowResult:
        """Handle zones input."""
        errors: dict[str, str] = {}
    ...
    async def async_step_select_zones(self, user_input=None) -> ConfigFlowResult:
        """Handle zones input."""
        errors = {}
...
class JRiverOptionsFlowHandler(OptionsFlow):
    ...
    _browse_paths: list[BrowsePath]
    _extra_fields: list[str]
    _mac_addresses: list[str]
    _use_wol: bool
    _device_per_zone: bool
    _device_zones: list[str] | None = None
    ...
    async def async_step_init(
        ...
        self._device_per_zone: bool = self._get_existing(CONF_DEVICE_PER_ZONE, False)
        self._device_zones: list[str] | None = self._get_existing(CONF_DEVICE_ZONES, [])
        ...
        if self._ms and self._device_per_zone is True:
            return await self.async_step_select_zones()
    ...
    async def _get_deselected_zones(self, ms: MediaServer) -> list[str]:
        """Find any zones that have not been selected."""
        existing_zones = self._device_zones if self._device_zones else []
        deselected_zones: list[str] = [
            z.name for z in await ms.get_zones() if z.name not in existing_zones
        ]
        return deselected_zones
    ...
    async def async_step_select_zones(
        ...
        deselected_zones: list[str] = await self._get_deselected_zones(self._ms)
        if not deselected_zones:
            return await self.async_step_macs()

        if user_input is not None:
            extra_zones: list[str] | None = user_input.get(CONF_DEVICE_ZONES, None)
            if extra_zones:
                if self._device_zones:
                    extra_zones = extra_zones + self._device_zones
                self._device_zones = extra_zones
            return await self.async_step_macs()

        schema = vol.Schema(
            {
                vol.Required(CONF_DEVICE_ZONES, default=[]): SelectSelector(
                    SelectSelectorConfig(multiple=True, options=deselected_zones)
                ),
            }
        )
        return self.async_show_form(
            step_id="select_zones", data_schema=schema, errors={}
        )
