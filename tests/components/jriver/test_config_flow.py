...
from components.jriver.coordinator import MediaServerData
from hamcws import (
    BrowseRule,
    CannotConnectError,
...
from homeassistant.components.jriver.const import (
    CONF_BROWSE_PATHS,
...
async def test_reconfigure_options(
    ...
    with (
        ...
        patch("homeassistant.components.jriver._get_ms", return_value=media_server),
        patch(
            "homeassistant.components.jriver.config_flow.JRiverOptionsFlowHandler._reload_ms",
            return_value=media_server,
        ),
    ):
